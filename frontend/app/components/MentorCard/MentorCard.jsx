...
const MentorCard = ({ mentor }) => {
  ...
  const startingPriceLabel =
    typeof mentor.startingPrice === 'number'
        ...
        <CardContent className='flex h-full flex-col p-4'>
          <div className='flex items-start gap-3.5'>
            ...
            <div className='min-w-0 flex-1'>
              <div className='flex items-start justify-between gap-2'>
                ...
                {mentor.rating !== null && mentor.rating !== void 0 ? (
          ...
          <p className='text-ink/80 mt-3 line-clamp-2 min-h-12 text-[15px] leading-relaxed'>
            {mentor.headline?.trim() ||
              'Xem h\u1ED3 s\u01A1 \u0111\u1EC3 t\xECm hi\u1EC3u th\xEAm v\u1EC1 mentor n\xE0y.'}
          </p>
          ...
          <div className='mt-4 grid grid-cols-3 divide-x divide-slate-200 rounded-xl bg-slate-50 py-3.5'>
            <QuickStat
              icon={<Clock3 size={16} />}
              label='Khả dụng'
              value={nextAvailability ? 'C\xF3 l\u1ECBch g\u1EA7n' : 'Xem l\u1ECBch'}
            />
            <QuickStat
              icon={<GraduationCap size={16} />}
              label={offerings.length ? 'Kh\xF3a h\u1ECDc' : 'H\u1ECDc ph\xED'}
              value={
                offerings.length
                  ? `${offerings.length} \u0111ang m\u1EDF`
                  : startingPriceLabel || 'Ch\u01B0a m\u1EDF'
              }
            />
            <QuickStat
            ...
            {offerings.length ? (
              ...
              <div className='px-3 py-4 text-center'>
                <p className='text-ink text-sm font-bold'>
                  {startingPriceLabel
                    ? `T\u1EEB ${startingPriceLabel}`
                    : 'Ch\u01B0a c\xF3 kh\xF3a h\u1ECDc \u0111ang m\u1EDF'}
                </p>
