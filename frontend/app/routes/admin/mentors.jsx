...
function AdminMentorsPage() {
  ...
  const [rejectVerifyId, setRejectVerifyId] = useState(null)
  const [approveId, setApproveId] = useState(null)
  const [rejectApproveId, setRejectApproveId] = useState(null)
  const [suspendId, setSuspendId] = useState(null)
  const [reactivateId, setReactivateId] = useState(null)
  const [viewMentorId, setViewMentorId] = useState(null)
  const [viewVerificationId, setViewVerificationId] = useState(null)
  ...
  const { data: directoryData } = useAdminMentorsQuery({
    approvalStatus: directoryFilter === 'ALL' ? void 0 : directoryFilter,
    search: searchQuery,
    page: 1,
    size: 50
  })
  ...
  const handleVerifyReject = (reason) => {
  ...
  const handleApproveReject = (reason) => {
    ...
    >
      <div className='space-y-8'>
                      ...
                      <div className='flex flex-wrap items-start justify-between gap-3'>
                        <div>
                          ...
                          <p className='text-muted mt-1 text-sm'>
                            {mentor.headline || 'Ch\u01B0a c\u1EADp nh\u1EADt ti\xEAu \u0111\u1EC1'}
                          </p>
                        </div>
                        <div className='flex flex-wrap gap-2'>
        ...
        <WorkspacePanel
          ...
          action={
            <StatusFilterPills
              onValueChange={(val) => setDirectoryFilter(val)}
...
export { AdminMentorsPage as default, meta }
