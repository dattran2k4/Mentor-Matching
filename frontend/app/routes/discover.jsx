...
import { AdvancedMentorFiltersDrawer } from '@/components/AdvancedMentorFiltersDrawer'
...
const meetingTypeOptions = [
  ...
  { label: 'Hybrid', value: 'HYBRID', helper: 'K\u1EBFt h\u1EE3p online v\xE0 tr\u1EF1c ti\u1EBFp' }
]
const genderOptions = [
...
const Discover = () => {
  ...
  const filterDetails = useMemo(() => {
    ...
    const addFilterDetail = (key, value, label) => {
  ...
  const handleDrawerFiltersApply = (values) => {
    const updates = {
      districtId: null,
      gender: null,
      meetingType: null
    ...
    <div className='space-y-6'>
          ...
          <div>
            <h2 className='text-ink text-lg font-semibold'>
              {mentorsQuery.isLoading && !mentorPage
                ? '\u0110ang t\xECm mentor ph\xF9 h\u1EE3p'
                : `${totalItems} mentor ph\xF9 h\u1EE3p`}
            </h2>
          ...
          <div className='flex flex-wrap items-center gap-2'>
            ...
            <div className='flex flex-wrap rounded-xl border border-slate-200 bg-white p-1'>
              {sortOptions.map((option) => (
                <button
                  key={option.key}
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${activeSort.key === option.key ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:text-slate-900'}`}
                  type='button'
      ...
      {filtersOpen ? (
        <AdvancedMentorFiltersDrawer
          ...
          meetingTypeOptions={meetingTypeOptions.map((option) => ({
...
function resolveSortOption(sortBy, sortDir) {
  return (
