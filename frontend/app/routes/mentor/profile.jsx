...
const profileTabs = [
  { key: 'general', label: 'Th\xF4ng tin chung', icon: BookOpenText },
  { key: 'traits', label: 'N\u1ED5i b\u1EADt & \u0110\u1EB7c \u0111i\u1EC3m', icon: Sparkles },
  { key: 'achievements', label: 'Th\xE0nh t\u1EF1u', icon: Award },
...
const achievementTypeOptions = [
  ...
  { label: 'Th\xE0nh t\xEDch h\u1ECDc t\u1EADp', value: 'EXAM_SCORE' },
  { label: 'Cu\u1ED9c thi', value: 'COMPETITION' },
  { label: 'D\u1EF1 \xE1n', value: 'PROJECT' },
  { label: 'Kinh nghi\u1EC7m th\u1EF1c t\u1EBF', value: 'WORK_EXPERIENCE' }
]
function mapMentorToFormValues(currentMentor) {
  return {
...
function getProfileSaveErrorMessage(error) {
  if (axios.isAxiosError(error)) {
    ...
    return (
      ...
      'Kh\xF4ng th\u1EC3 l\u01B0u h\u1ED3 s\u01A1 mentor l\xFAc n\xE0y.'
    )
  }
  return 'Kh\xF4ng th\u1EC3 l\u01B0u h\u1ED3 s\u01A1 mentor l\xFAc n\xE0y.'
}
...
function sortNumberList(items) {
  return [...items].sort((left, right) => left - right)
}
function areTraitValuesEqual(left, right) {
...
function createEmptyAchievementDraft() {
  return {
    ...
    issuer: '',
    achievedYear: String(/* @__PURE__ */ new Date().getFullYear()),
    proofUrl: '',
    verified: false,
    isNew: true
  }
}
function mapAchievementToDraft(achievement) {
  return {
...
function mapTraitLabels(traits, personalityOptions, highlightOptions) {
  if (!traits) return []
...
function buildAchievementPayload(draft) {
  return {
...
function buildProfileExpertise(currentMentor) {
  ...
  return (
    value ||
    'B\u1ED5 sung v\u1ECB tr\xED hi\u1EC7n t\u1EA1i, h\u1ECDc v\u1EA5n ho\u1EB7c chuy\xEAn m\xF4n \u0111\u1EC3 h\u1ED3 s\u01A1 r\xF5 h\u01A1n.'
  )
}
function buildProfileMeta(currentMentor) {
  const location =
  ...
  return (
    value ||
    'B\u1ED5 sung h\xECnh th\u1EE9c h\u1ECDc, kinh nghi\u1EC7m v\xE0 khu v\u1EF1c \u0111\u1EC3 h\u1ED3 s\u01A1 r\xF5 r\xE0ng h\u01A1n.'
  )
}
function buildApprovalSummary(currentMentor) {
  if (currentMentor.approvalNote?.trim()) {
    return `Ghi ch\xFA duy\u1EC7t: ${currentMentor.approvalNote.trim()}`
  }
  if (currentMentor.approvalStatus === 'APPROVED') {
    return 'H\u1ED3 s\u01A1 \u0111\xE3 \u0111\u01B0\u1EE3c duy\u1EC7t v\xE0 s\u1EB5n s\xE0ng hi\u1EC3n th\u1ECB c\xF4ng khai.'
  }
  if (currentMentor.approvalStatus === 'PENDING') {
    return 'H\u1ED3 s\u01A1 \u0111ang ch\u1EDD duy\u1EC7t. Gi\u1EEF ph\u1EA7n gi\u1EDBi thi\u1EC7u v\xE0 offerings th\u1EADt r\xF5 r\xE0ng.'
  }
  if (currentMentor.approvalStatus === 'REJECTED') {
    return 'H\u1ED3 s\u01A1 t\u1EEBng b\u1ECB t\u1EEB ch\u1ED1i duy\u1EC7t. H\xE3y r\xE0 so\xE1t l\u1EA1i th\xF4ng tin c\xF4ng khai.'
  }
  return 'H\u1ED3 s\u01A1 hi\u1EC7n \u0111ang t\u1EA1m d\u1EEBng hi\u1EC3n th\u1ECB c\xF4ng khai.'
}
function buildVerificationSummary(currentMentor, verification) {
  if (verification?.rejectionReason?.trim()) {
    return `X\xE1c minh c\u1EA7n b\u1ED5 sung: ${verification.rejectionReason.trim()}`
  }
  if (verification?.verifiedAt) {
    return `Gi\u1EA5y t\u1EDD \u0111\xE3 \u0111\u01B0\u1EE3c x\xE1c minh v\xE0o ${formatVerificationDate(verification.verifiedAt)}.`
  }
  if (currentMentor.verificationStatus === 'PENDING') {
    return 'H\u1ED3 s\u01A1 x\xE1c minh \u0111ang \u0111\u01B0\u1EE3c \u0111\u1ED9i ng\u0169 ki\u1EC3m tra.'
  }
  if (currentMentor.verificationStatus === 'UNVERIFIED') {
    return 'B\u1EA1n ch\u01B0a g\u1EEDi \u0111\u1EE7 th\xF4ng tin x\xE1c minh cho h\u1ED3 s\u01A1 hi\u1EC7n t\u1EA1i.'
  }
  return 'Tr\u1EA1ng th\xE1i x\xE1c minh \u0111ang \u0111\u01B0\u1EE3c \u0111\u1ED3ng b\u1ED9 t\u1EEB backend.'
}
...
function MentorProfilePage() {
  ...
  const [draftTraitValues, setDraftTraitValues] = useState(null)
  const [traitsSuccessMessage, setTraitsSuccessMessage] = useState(null)
  const [traitsErrorMessage, setTraitsErrorMessage] = useState(null)
  const [draftAchievements, setDraftAchievements] = useState(null)
  const [achievementsSuccessMessage, setAchievementsSuccessMessage] = useState(null)
  const [achievementsErrorMessage, setAchievementsErrorMessage] = useState(null)
  const [savingAchievementLocalId, setSavingAchievementLocalId] = useState(null)
  const [deletingAchievementLocalId, setDeletingAchievementLocalId] = useState(null)
  ...
  const offeringSummaries = useMemo(
    () =>
      (profileBundle?.subjects ?? []).map((subject) => ({
        ...subject,
        label: `${subject.subjectName} \xB7 ${subject.gradeName}`
      })),
  ...
  const toggleTrait = (group, optionId) => {
    setTraitsSuccessMessage(null)
    setTraitsErrorMessage(null)
  ...
  const handleTraitsSubmit = (event) => {
    event.preventDefault()
    setTraitsSuccessMessage(null)
    setTraitsErrorMessage(null)
    updateCurrentMentorTraitsMutation.mutate(traitValues, {
      onSuccess: ({ traits, message }) => {
        ...
        setTraitsSuccessMessage(
          message ||
            '\u0110\xE3 c\u1EADp nh\u1EADt n\u1ED5i b\u1EADt v\xE0 \u0111\u1EB7c \u0111i\u1EC3m th\xE0nh c\xF4ng.'
        )
      },
      onError: (error) => {
  ...
  const updateAchievementDraft = (localId, patch) => {
    setAchievementsSuccessMessage(null)
    setAchievementsErrorMessage(null)
  ...
  const handleAchievementFileChange = (localId) => async (event) => {
  ...
  const handleSaveAchievement = (draft) => {
    const payload = buildAchievementPayload(draft)
    if (!payload.title.trim()) {
      setAchievementsErrorMessage(
        'T\xEAn th\xE0nh t\u1EF1u l\xE0 b\u1EAFt bu\u1ED9c tr\u01B0\u1EDBc khi l\u01B0u.'
      )
      return
    }
    setAchievementsSuccessMessage(null)
    ...
    if (draft.id) {
      updateAchievementMutation.mutate(
        ...
        {
          onSuccess: ({ message }) =>
            onSuccess(message || '\u0110\xE3 c\u1EADp nh\u1EADt th\xE0nh t\u1EF1u.'),
          onError
        }
      )
      return
    ...
    createAchievementMutation.mutate(payload, {
      onSuccess: ({ message }) =>
        onSuccess(message || '\u0110\xE3 th\xEAm th\xE0nh t\u1EF1u m\u1EDBi.'),
      onError
    })
  }
  const handleDeleteAchievement = (draft) => {
    setAchievementsSuccessMessage(null)
    setAchievementsErrorMessage(null)
    if (!draft.id) {
      setDraftAchievements((currentDrafts) =>
    ...
    deleteAchievementMutation.mutate(draft.id, {
      onSuccess: ({ message }) => {
        setAchievementsSuccessMessage(message || '\u0110\xE3 x\xF3a th\xE0nh t\u1EF1u.')
        setDraftAchievements(null)
        setDeletingAchievementLocalId(null)
      },
      onError: (error) => {
                ...
                <div className='space-y-2'>
                  ...
                  <p className='text-slate-600'>
                    {currentMentor.headline?.trim() ||
                      'B\u1ED5 sung headline \u0111\u1EC3 h\u1ECDc vi\xEAn hi\u1EC3u nhanh th\u1EBF m\u1EA1nh c\u1EE7a b\u1EA1n.'}
                  </p>
                ...
                <CardContent className='space-y-8 p-6'>
                  ...
                  <div className='grid gap-4 md:grid-cols-2'>
                    ...
                    <InfoMiniCard
                      description={
                        traitLabels.length
                          ? `C\xE1c th\u1EBB \u0111\xE3 ch\u1ECDn hi\u1EC7n t\u1EA1i: ${traitLabels.join(', ')}.`
                          : 'Ch\u01B0a c\xF3 th\u1EBB n\xE0o \u0111\u01B0\u1EE3c ch\u1ECDn \u0111\u1EC3 hi\u1EC3n th\u1ECB tr\xEAn h\u1ED3 s\u01A1 v\xE0 b\u1ED9 l\u1ECDc h\u1ECDc vi\xEAn.'
                      }
                      icon={Lightbulb}
                            ...
                            <CardContent className='space-y-5 p-4 md:p-5'>
                                ...
                                <div className='space-y-2'>
                                  ...
                                  <AppSelect
                                    ...
                                    options={Array.from({ length: 16 }, (_, index) => {
                                      const year = String(
                                        /* @__PURE__ */ new Date().getFullYear() - index
                                      )
                              ...
                              <div className='flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between'>
                                ...
                                <div className='flex flex-wrap justify-end gap-3'>
                                  ...
                                  >
                                    {achievement.isNew
                                      ? 'L\u01B0u th\xE0nh t\u1EF1u m\u1EDBi'
                                      : 'C\u1EADp nh\u1EADt th\xE0nh t\u1EF1u'}
                                  </Button>
                                </div>
                              </div>
                            </CardContent>
                ...
                {offeringSummaries.length ? (
                  <div className='space-y-3'>
                    {offeringSummaries.map((offering, index) => {
                      const iconMap = [Sigma, GraduationCap, Atom, FlaskConical]
                                ...
                                <div className='flex flex-wrap items-center gap-2 text-sm text-slate-600'>
                                  ...
                                  <span>
                                    {offering.teachingNote?.trim() ||
                                      'Thi\u1EBFt l\u1EADp l\u1ED9 tr\xECnh v\xE0 h\xECnh th\u1EE9c h\u1ECDc s\u1EBD hi\u1EC3n th\u1ECB \u1EDF \u0111\xE2y.'}
                                  </span>
                                </div>
                              </div>
                            </div>
                    ...
                    <div className='grid gap-3 md:grid-cols-4'>
                      <div className='space-y-2'>
                        ...
                        <AppSelect
                          ...
                          options={[
                            { label: 'To\xE1n', value: 'toan' },
                            { label: 'V\u1EADt l\xFD', value: 'vat-ly' },
                            { label: 'H\xF3a h\u1ECDc', value: 'hoa-hoc' }
                          ]}
                      ...
                      <div className='space-y-2'>
                        ...
                        <AppSelect
                          ...
                          options={[
                            ...
                            { label: 'N\xE2ng cao', value: 'advanced' },
                            { label: 'Chuy\xEAn s\xE2u', value: 'expert' }
                          ]}
                          placeholder='Cơ bản'
                          value={quickAddLevel}
                        />
          ...
          <Card className='overflow-hidden rounded-[26px] border-slate-200 shadow-none'>
            ...
            <CardContent className='space-y-4 p-6 pt-0'>
              ...
              <div className='space-y-1'>
                ...
                <p className='text-sm text-slate-500'>
                  {currentMentor.headline?.trim() ||
                    'Mentor \u0111ang ho\xE0n thi\u1EC7n headline c\xF4ng khai.'}
                </p>
              </div>
              <div className='flex flex-wrap gap-2'>
...
function TraitOptionSection({ title, description, options, selectedIds, emptyMessage, onToggle }) {
  return (
...
function AchievementProofPanel({ proofUrl, fileInputId, onFileChange, onClear }) {
  return (
        ...
        >
          ...
          <span className='min-w-0 space-y-2'>
            <span className='block text-base font-semibold text-slate-800'>
              {proofUrl
                ? '\u1EA2nh minh ch\u1EE9ng \u0111\xE3 ch\u1ECDn'
                : 'T\u1EA3i \u1EA3nh minh ch\u1EE9ng'}
            </span>
            <span className='block text-sm leading-relaxed text-slate-500'>
              {proofUrl
                ? 'B\u1EA1n c\xF3 th\u1EC3 b\u1EA5m \u0111\u1EC3 thay \u1EA3nh kh\xE1c ho\u1EB7c xo\xE1 \u1EA3nh hi\u1EC7n t\u1EA1i.'
                : '\u01AFu ti\xEAn \u1EA3nh r\xF5 n\xE9t c\u1EE7a ch\u1EE9ng ch\u1EC9, b\u1EB1ng c\u1EA5p ho\u1EB7c gi\u1EA5y x\xE1c nh\u1EADn li\xEAn quan.'}
            </span>
            {proofUrl ? (
...
export { MentorProfilePage as default, meta }
